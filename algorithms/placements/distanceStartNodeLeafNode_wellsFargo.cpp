//given a graph and a start node, find the shortest distance from the start node to the leaf node where leaf node is the node connected to only one other node.(BFS)
